...
def calculate_combined_seq(uniprot_id, rowname=""):
    identifier, pdb_path = find_pdb_file(uniprot_id, rowname)
    process_id = current_process().pid
    parsed_seqs = get_struc_seq("bin/foldseek", pdb_path, ["A"], process_id=process_id, plddt_mask=True)
...
def find_pdb_file(uniprot_id, rowname):
    if uniprot_id == "NA":
        pdb_file = "colabfold/" + str(file_row) + ".pdb"
        identifier = rowname
    else:
        pdb_file = "AFDB/" + uniprot_id + ".pdb"
...
def main():
    ...
    if 'BIOSNAP' not in args.input_csv:
        df['rowname'] = '_'.join(args.input_csv.split("/")[-2:]) + df.index.astype(str)
    else:
        df['rowname'] = '_'.join(['BIOSNAP',args.input_csv.split("/")[-1]]) + df.index.astype(str)
    na_rownames = df[df["uniprot_id"] == "NA"]["rowname"]
    # create a list of tuples with the uniprot ID and the row index
    foldseek_calc_list = list((uid, "") for uid in uniprot_ids)  + list(("NA", narow) for narow in na_rownames)
    ...
    with Pool(processes=args.processes) as p:
        results = p.starmap(calculate_combined_seq, foldseek_calc_list)
